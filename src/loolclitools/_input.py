...
def askpath(prompt: str, /, must_be_file: bool = False, must_be_dir: bool = False) -> str:
    ...
    def err(msg):
        return out(f"\x1b[B\x1b[2C\x1b[J\x1b[91m{msg}\x1b[0m\x1b[u\x1b[K")
    ...
    try:
        while True:
            if prev_pos != os.get_terminal_size():
                out(
                    f"\x1b[s\x1b[{start_pos}H\x1b[1J\x1b[u",
                    HEADER(),
...
def notepad_input(
    ...
    notepad_path: os.PathLike = NOTEPAD_PATH,
) -> BufferedReader:
    """
    ...
    try:
        ...
        out(
            "\x1b[u\x1b[4B\x1b[K  \x1b[90mThe process terminated too quickly, assuming the file was handeled in another instance."
            "\x1b[u\x1b[K  \x1b[93mPress any key to continue . . . \x1b[?25h",
            flush=True,
        )
        getch()
    finally:
